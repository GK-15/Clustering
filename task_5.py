...
X_phonemes_1_2 = np.zeros((np.sum(phoneme_id==p_id), 2))
i=0;
for j in range(len(phoneme_id)):
    if phoneme_id[j] == p_id:
        X_phonemes_1_2 = X_full[phoneme_id == p_id]
        i=i+1
...
plot_data_3D(X=X_phonemes_1_2, title_string=title_string, ax=ax1)
