...
f1 = np.linspace(min_f1, max_f1, N_f1)
f2 = np.linspace(min_f2, max_f2, N_f2)
a, b = np.meshgrid(f1, f2)

a1 = a.flatten()
b1 = b.flatten()
samples = np.stack((a1, b1)).transpose()

GMM_Model='data/GMM_params_phoneme_01_k_06.npy';
...
GMM_Model='data/GMM_params_phoneme_02_k_06.npy';
Model = np.load(GMM_Model, allow_pickle=True)
M2=Model.tolist()
mu2 = M2.get('mu')
s2 = M2.get('s')
...
Z1 = get_predictions(mu1, s1, p1, samples)
Z2 = get_predictions(mu2, s2, p2, samples)

c1 = np.max(Z1, axis=1)
c2 = np.sum(Z2, axis=1)

pred = np.ones(len(samples)) * 2
pred[c1 >= c2] = 1
M = pred.reshape(N_f2, N_f1)
